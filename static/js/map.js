...
function spawnLabel(spawnpoint_id, disappear_time, latitude, longitude) {
  ...
  var str;
    str = `
      <div>
        <b>Spawn point <b>${spawnpoint_id}</b></b>
      </div>
      <div>
        Location: ${latitude.toFixed(6)}, ${longitude.toFixed(7)}
      </div>
  ...
  if ((notifiedPokemon.indexOf(item.pokemon_id) > -1) || (jQuery.inArray(item.pokemon_rarity, regularPokemon) < 0)) {
    if (!skipNotification) {
      if (Store.get('playSound')) {
        audio.play();
        sendNotification('A rare wild ' + item.pokemon_name + ' appeared!', 'Click to load map', 'static/icons/' + item.pokemon_id + '.png', item.latitude, item.longitude);
